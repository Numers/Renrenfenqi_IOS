//
//  CurMonthRepaymentViewController.m
//  renrenfenqi
//
...
@interface CurMonthRepaymentViewController ()
{
...
@implementation CurMonthRepaymentViewController
...
-(void)getBillInfo
{
    [[RFBillHomeManager defaultManager] getBillInfoSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
...
-(IBAction)clickPayBtn:(id)sender
{
    ...
    BillPaymentViewController *billPaymentVC = [mainStoryBoard instantiateViewControllerWithIdentifier:@"BillPaymentIdentifier"];
    [billPaymentVC setCalRepaymentMoney:[NSNumber numberWithFloat:[calRepaymentMoney floatValue]]];
    [AppUtils pushPage:self targetVC:billPaymentVC];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ((indexPath.row > 3) && (billArray.count > 0)) {
        ...
        if ([bill.type isEqualToString:@"late_fee"]) {
            ...
            [lateFeeVC updateUIWithBill:bill];
