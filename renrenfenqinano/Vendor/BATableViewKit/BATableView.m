...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.tableViewIndex = [[BATableViewIndex alloc] initWithFrame:(CGRect){frame.size.width - 25,0,25,frame.size.height}];
...
-(void)scrollToRowAtIndexPath:(NSIndexPath *)indexPath atScrollPosition:(UITableViewScrollPosition)position animated:(BOOL)animate
{
...
- (void)tableViewIndex:(BATableViewIndex *)tableViewIndex didSelectSectionAtIndex:(NSInteger)index withTitle:(NSString *)title
{
    if ([self.tableView numberOfSections] > index && index > -1){   // for safety, should always be YES
        [self.delegate scrollToRowWithHeaderTitle:title WithSection:index];
