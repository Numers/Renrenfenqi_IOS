...
//        imageView.image = item.image;
        if (item.url) {
            [imageView sd_setImageWithURL:[NSURL URLWithString:item.url] placeholderImage:[UIImage imageNamed:@"list_body_nopic_n"]];
        }
        else
        {
            imageView.image = item.image;
...
//    }
    ...
    if (tempPageIndex != self.pageControl.currentPage) {
        if ([self.delegate respondsToSelector:@selector(foucusImageFrame:didSelectPage:)]) {
            [self.delegate foucusImageFrame:self didSelectPage:(int)self.pageControl.currentPage];
        }
    }
